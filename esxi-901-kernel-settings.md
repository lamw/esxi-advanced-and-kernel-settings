# ESXi 9.0.1 Build 24957456 Kernel Settings
...
| atsDisallowPCIeVidDid | Colon separated <vendorID>:<deviceID> in hex of the devicefor which enabling ATS is disallowed |  |  |  |
...
| BOOTUUID | Boot partition UUID. |  | 912d4da4f7bd4908aee934403586dc52 |  |
...
| cxlStressOpts | Stress options that are used in CXL testing | 0 | 0 | 0 |
...
| desiredPcpusPerDomainDynamic | Desired number of PCPUs per cache domain when dynamic is set in numCacheDomains (=0). | 64 | 64 | 64 |
...
| earlyUWDumpEnabled | Enable userworld coredumps during boot. | FALSE | FALSE | FALSE |
...
| enableACSDTP2P | Enable ACS Direct Translated P2P. | TRUE | TRUE | TRUE |
| enableAllPCIESegments | Enable PCIE segment numbers > 0 (segment 0 is always enabled) | TRUE | TRUE | TRUE |
| enableAtomicOpsForAMDGPUs | Enable PCI quirk that enables Atomic Ops for AMD GPUs. | TRUE | TRUE | TRUE |
| enableCXL | Enable discovery of CXL devices | FALSE | FALSE | FALSE |
| enableCXLOSF | Enable OS first model for CXL _OSC method | FALSE | FALSE | FALSE |
...
| enableVmBwMonitoring | Enable/Disable vm level bandwidth monitoring | FALSE | FALSE | FALSE |
...
| execInstalledOnly | Execute only those files that have been installed via a vib package and have not been modified. | TRUE | TRUE | TRUE |
...
| haltOnBootDeviceLoss | Halt the system if the boot device accessibility is lost | FALSE | FALSE | FALSE |
...
| heapPoisonPaddingBytes | Number of additional poisoned red zone bytes for allocations when heapMetadataProtect is enabled (up to 64 KB) | 4 | 4 | 4 |
...
| iptIdleSize | Idle processor trace size (pages). | 0 | 0 | 0 |
| iptIdleTiming | Idle processor trace timing params (mtcFreq:psbFreq:cycThresh:enableTSC). | 0:0:0:1 | 0:0:0:1 | 0:0:0:1 |
| iptNumContextsMax | Maximum number of IPT contexts. | 100 | 100 | 100 |
...
| latencyOptimMode | If set to true, explicitly enable CPU Latency Optimized Mode only on Intel GNR systems. If false, use BIOS settings. | TRUE | TRUE | TRUE |
...
| maxNUMAHistoryEntries | Maximum number of NUMA scheduler histories tracked | 32 | 32 | 32 |
...
| numCacheDomains | Cache domains per LLC for cache idle balancing. 0 = dynamic only if number of cores per LLC is greater than maxCoresPerDomainDynamic. | 0 | 0 | 0 |
...
| protectPages | Write protect free pages when running in a VM | FALSE | FALSE | FALSE |
...
| readHMATOnBoot | Read and parse the HMAT table during ACPI Initialization | FALSE | FALSE | FALSE |
...
| seamModForceType | Force use of `debug` or `prod` SEAM Module. |  |  |  |
...
| useHWP | Use Intel Hardware P-states if available for ESXi power management. If set to FALSE, use legacy P-states if available. This option is supported on Intel Sapphire Rapids onward. | TRUE | TRUE | TRUE |
...
| usePRM | Use Platform Runtime Mechanism if available | FALSE | FALSE | FALSE |
...
| vmkacMaxObjects | Maximum number of security object types | 128 | 128 | 128 |
...
| vtdMaxDomains | Upper limit for number of VT-d domains supported. | 2048 | 2048 | 2048 |
